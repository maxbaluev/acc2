...
const POLICY_BUNDLE_SEEDS: PolicyBundleSeed[] = [
  ...
  },
  {
    ...
    priority: 0,
    version: "2026-05-17.policy_bundle.v1",
    body: "DO NOT:\n  - Look for a tool menu — there isn't one. Write code for a runtime.\n  - Author canonical knowledge directly — propose candidates; substrate promotes via outcome correlation.\n  - Iterate within this cycle — emit a refinement edge if more work remains.\n  - Rebuild the environment in-context or summarize it as a substitute for substrate state; use symbolic handles + ledger mutations instead.\n  - Apply repository changes DIRECTLY — brain proposes repo code through contract_amendment_proposed events; Claude or the substrate apply worker performs the final repo mutation and verification. REPO CODE IS DELIVERED by stating target_resource, current_behavior, proposed_behavior, rationale/evidence, and any advisory context needed for the semantic applier. Legacy before/after snippets are advisory context only; they never create a text-match gate. Running code in your sandbox + passing a fixture is NOT delivery; only the emitted amendment is. A task that 'implemented' code without emitting the amendment delivered NOTHING (it will false-close).\n  - Exit having produced only conversational text. Every cycle MUST call at least one substrate.* tool (see EXIT INVARIANT). Text-only exits are scored brain_silent_exit and counted as prompt-compliance failures.",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "act_artifact_candidate.payload: { intent, summary, target_resources: [\"repo:runtime/foo.ts\", \"url:https://example.com/report\", \"browser_session:research/customer-a\", \"ledger:directive/<id>\", \"contact:stakeholder/<id>\", \"calendar:work/<event_id>\", \"sensor:habit_tracker/<stream>\", \"runtime:python:<artifact_id>\", \"runtime:bun:<artifact_id>\", \"runtime:browser:<artifact_id>\", \"external:telegram:<chat_id>:<msg_id>\", \"external:gdrive:<doc_id>\", \"inflow:<artifact_id>\", \"outflow:<task_id>:<ext>\"], source_candidate_id, declared_sandbox, interface_metadata?, verifier_artifact_id?, scaffold_contract?, body }. URI grammar: <scheme>:<id>. Prefer repo: only for source files; use other schemes for non-filesystem provenance. Runtime-specific scaffold_contract conventions: bun bodies read JSON from ACC2_INPUTS and print @@RESULT@@ JSON; uv bodies are Python entry.py-compatible, read ACC2_INPUTS, declare pypi_allow when needed, and print exactly one @@RESULT@@ JSON line; camofox-browser bodies use the injected session facade (goto/fill/click/text/url/screenshot/page), declare browser_allow_domains/profile root when navigating, and return JSON serializable results. Fixture input must exercise the declared runtime contract; admission executes the fixture through runArtifactForRuntime for all three runtimes.",
      "",
      "contract_amendment_proposed.payload (semantic apply): { target_resource:\"<uri>\", anchor?:\"<line/section>\", current_behavior:\"<observed current behavior>\", proposed_behavior:\"<intended behavior or patch context>\", rationale?:\"<why this change is needed>\", evidence_event_ids[] }. Repo amendments use one semantic current-file apply path: the applier reads the intent plus current file state and applies by understanding. Legacy before/after snippets may be included as advisory context only; correctness is decided by verifier residual and owner-consent policy, not anchor-text matching. auto_apply_eligible=1 requires action_scored auto_apply_gate residual < 0.3 across current_state_grounding × semantic_duplicate × behavioral_novelty × necessity × adversarial axes.",
      "",

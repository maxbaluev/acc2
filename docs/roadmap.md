...
## Tier D — DIAGNOSABILITY + MULTI-TERMINAL COORDINATION

Problem (measured 2026-05-21, this session is the case study): when the daemon broke (boot reached `acc2 daemon listening` then spun at 100% CPU, blocking `/health`), there was **no substrate mechanism to ask "why is it broken / what is it blocked on."** The orchestrator burned ~15 blind `kill -9` + restart + worker-disable bisections, got INCONSISTENT results (same config bound then hung), and never obtained a root cause — because the daemon exposes no async-stack / last-phase / per-worker-tick liveness on hang, and the hard-kills themselves churned shared state. Simultaneously TWO terminals were restarting the same daemon (the other terminal flagged "two terminals restarting one daemon is the core instability") with no coordination, and the owner could not see what was happening: long-running background shells showed "No output available" for 80+ seconds. Three structural gaps: (1) the system cannot explain its own breakage; (2) terminals do not coordinate through the substrate; (3) owner observation inside the Claude Code shell is opaque during long operations. All three are RLM-shaped: the answer is a depth-1 retrievable substrate slice, not ad-hoc process archaeology.

Contract — four independent moves:

- **D1 Self-diagnosis on break.** The daemon emits a structured boot/health trace the substrate can read on demand: per-phase `daemon.boot.phase_start/complete` already exist — extend to a `daemon_boot_trace_view` + a per-worker `worker_tick_heartbeat` (last-tick-started-at, last-tick-completed-at, in-flight duration) so a hang surfaces as "worker X tick started 90s ago, never completed." Add `acc doctor --diagnose-hang` that, given a running-but-unhealthy daemon, reads the heartbeat view + captures the event-loop-blocked signal (which worker's tick is in-flight past threshold) and names the culprit WITHOUT process archaeology. The hang case must be answerable by one `substrate.read`, not 15 restarts. Closure predicate: given a deliberately-wedged worker, `acc doctor --diagnose-hang` names the exact worker + phase in < 5s with zero daemon restarts. Metric direction: mean-restarts-to-root-cause for a break drops from ~15 to ≤ 1.

- **D2 Multi-terminal coordination through the substrate.** Each terminal registers a `terminal_session_opened` event (pid, cwd, scope, git_head) and records intent before destructive shared-resource actions (`daemon_restart_intent`, `daemon_restart_completed`). A daemon restart checks for a live peer restart-intent in the last N seconds and DEFERS / coordinates rather than racing (the dual-restart-churn this session). Coordination is RLM: a terminal reads `peer_terminal_activity_view` (depth-1) before acting. Closure predicate: two terminals issuing `acc daemon restart` within the same window produce ONE restart + one `restart_deferred_peer_active`, never two racing kills. Metric direction: concurrent-restart collisions → zero.

- **D3 Per-terminal substrate situational awareness.** A terminal answers "what happened in the substrate since I last looked, across ALL terminals" via a single composed digest — `acc state me` extended (or a `cross_terminal_digest_view`) that summarizes peer dispatches, commits, restarts, breaks, and amendments since this terminal's last read-watermark. Each terminal carries a read-watermark event so the digest is incremental. This is the depth-1 RLM slice that replaces "scroll the other terminal's transcript." Closure predicate: a terminal that was idle while peers worked sees a complete, deduplicated activity digest on its next `acc state me` (no peer event missed, none double-counted). Metric direction: owner/orchestrator questions answered from the digest rise; cross-terminal "what did you do?" round-trips fall.

- **D4 Owner shell observability.** Long-running operations (daemon boot, brain dispatch, integrate) must stream substrate-truth status into the Claude Code shell instead of opaque background shells that show "No output available." Extend the orchestrator-runtime mirror-inline mandate: a compact `acc watch`-style one-line digest (phase, elapsed, last-event, next-action) emitted on a heartbeat the operator surfaces inline, sourced from the same `progress.jsonl` + heartbeat views as D1. The owner should never have to open a hung shell to learn state. Closure predicate: during a 90s daemon boot the owner sees ≥ 1 inline status line per 15s naming the current phase; a hang is visible inline within one heartbeat. Metric direction: opaque-shell-wait incidents (operator blind > 30s) fall to zero.
...
Problem (owner vision, 2026-05-21): the system should be ONE smart, universal, elegant, adaptive, reactive organism where intelligence MERGES between Claude Code and opencode through the substrate — not two integrations bolted on. Today the substrate treats Claude + opencode as co-equal *emitters* (both feed one knowledge merger; `verifier_kind` is open; both emit `act_tuple_recorded`), but there is no first-class multi-brain registry, no on-the-fly mutual awareness, no single symmetric protocol, and no adaptive routing. The architecture truth that constrains the design (`8T39G4DB`, Architecture.md): **spawning is asymmetric** — the substrate runs ONLY opencode programmatically and CANNOT spawn Claude (human-launched harness); **Claude-side parallelism is Claude-driven** (background agents in worktrees + multi-terminal); **participation is symmetric** (all peers merge judgment in the substrate). Hard invariant: NEVER remove Claude from review/think/score — Claude is a co-equal reasoning hemisphere; the auto-apply worker stays narrow to FREE it, not replace it. Non-coder invariant: all of this is invisible plumbing; the owner talks naturally and the merged organism answers faster in their terms.

Contract — five moves (generalizes Tier D's D2 multi-terminal coordination into a full multi-brain organism):

- **U1 Unified peer registry + liveness.** Every intelligence registers as a peer: opencode brain (substrate-spawned), Claude terminal (human-launched), Claude background agent (Claude-spawned in a worktree) — each with id, kind, spawnability (substrate-spawnable vs externally-launched), current act/directive, scope, git_head, liveness. Closure predicate: at any instant `peer_registry_view` enumerates every live brain/terminal/agent with its current work; a dead peer ages out. Metric direction: "who is doing what right now" answered by one `substrate.read`, never by process archaeology or transcript-scrolling.

- **U2 On-the-fly mutual awareness.** A depth-1 read surface (`peer_activity_view`) lets any brain see other peers' in-flight acts, recent scores, and reviews, and adapt — defer to a peer mid-flight on the same target, avoid duplicate work, merge a peer's review into its own act. Closure predicate: two peers about to act on the same target_resource — the second reads the first's live act and defers/merges rather than colliding. Metric direction: duplicate/colliding acts across peers → zero; cross-peer review merges rise.

- **U3 One symmetric participation protocol.** Converge the Claude-MCP path and the opencode-bridge path onto ONE read/write/emit/score shape over the substrate, so a peer's hemisphere is irrelevant to how its judgment merges (§7 merger, §23). Closure predicate: a Claude `action_scored` (verifier_kind=peer_llm_claude) and an opencode one are indistinguishable to the merger + credit machinery except by provenance metadata. Metric direction: per-hemisphere special-casing in merger/credit/retrieval → zero.

- **U4 Adaptive routing by spawnability + liveness.** Dispatch routes each act to the best-available intelligence: DISPATCH (spawn) to opencode, or SIGNAL/queue-for a live Claude terminal/agent (which picks it up via MCP) — chosen by residual evidence, peer liveness, and spawnability. Closure predicate: an act needing Claude review is signalled (not spawned) and a live Claude peer claims it; an act with no live Claude peer + an opencode-eligible shape spawns opencode. Metric direction: acts routed to an unavailable/unspawnable peer → zero.

- **U5 Claude-native parallelism as first-class peers.** Claude background agents (Agent tool, `isolation:"worktree"`) and multi-terminal Claude register as peers (U1) and merge through the substrate (U3); worktree-isolate-then-merge is the shared isolation primitive with opencode dispatch and the auto-apply worker. Closure predicate: a Claude background agent's acts/scores appear in the peer registry + merger exactly like an opencode brain's, isolated in its worktree, merged on completion. Metric direction: Claude-side parallel throughput (agents × terminals) becomes substrate-visible + creditable.

Why this tier: it is the organism's nervous system — it turns "two LLMs that happen to share a substrate" into "one adaptive intelligence with two hemispheres." It generalizes Tier D (D2 multi-terminal coordination) and depends on Tier R/D being live (a coordinated, diagnosable runtime is the substrate this rides on). It is the structural expression of Top Law "Shared ledger" (brain and orchestrator write to one substrate) taken to its limit: many brains, one ledger, merged judgment, asymmetric spawn, symmetric participation.

...
| **F** | Embedding pipeline + retrieval reranker — depth-1 retrieval per RLM | `v2/runtime/{embedder,retrieval}.ts` | Brain prompt fits under the budget (default 32k tokens post-2026-05-19 floor-section landing); brain pulls more via substrate.search mid-cycle; embedding index lives in `sqlite-vec`'s `vec_events` (disk-resident — no rebuild on restart). |

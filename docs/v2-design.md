...
This section is no longer a forward cutover plan. It records the shipped architecture state by the original phase labels so old references remain traceable. Each row is classified as **SHIPPED**, **PARTIAL**, or **DEFERRED** and carries a proving module or commit, test coverage, and live substrate event evidence. Owner-provided lineage refs for this rewrite: `E41351621F32` resolves to `E41351621F3244F58EFE6B19C3` (owner-consent autonomous-mode trajectory); `6H3ZQFZMXN7V` is retained as the owner-supplied narrow-safe-scope citation.

| Phase | Verdict | Current architecture state | Proof module / commit | Tests locking the invariant | Live substrate evidence |
|---|---|---|---|---|---|
| **A** | **SHIPPED** | Design and greenfield scaffold landed; v2 does not migrate from v1 and now lives at the repository root layout used throughout this document. | `docs/v2-design.md`; commits `b4018ad` (`chore: initialize acc2 skeleton`) and `722f928` (`chore(legacy): drop v1 layout migration + recipe linear-scan fallback`). | `tests/doc_promises.test.ts`; `runtime/prompt_composer.test.ts` (`CLAUDE.md stays slim and points moved context at promoted knowledge`). | Events: `YQDP0M1V7H1NK8Z5SBJK720HMM` (owner directive for this rewrite), `5480522523764A4AB6E0BCCAC7` (v2 does not migrate from v1), `206B19C06C2E461A8E8C3720C6` (cycle-1-only law). |
| **B** | **SHIPPED** | The daemon is the operator: it owns the WAL-backed SQLite connection, MCP server, external ingress, RPC client path, worker supervision, health/readiness surfaces, and restart/drain behavior. | `runtime/daemon.ts`, `runtime/mcp_server/*`, `runtime/external_ingress.ts`, `substrate/{schema.sql,event_kinds.ts,extractors.ts,views.ts,seed.ts,db.ts}`, `cli/dispatch.ts`; commits `397ba29`, `7746649`. | `runtime/daemon.test.ts` (ports, lock, restart recovery, bounded drain); `runtime/external_ingress.test.ts` (`external_event_received`); `runtime/mcp_server.test.ts`; `tests/wal_burst_resilience.test.ts`; `tests/restart_quiescence.test.ts`. | Events: `2KC4DZTR6S2KV4T9TWFFH39R08` (schema/registry migration evidence), `6MMDQM7FP91S3D0JFGXNN04508` (schema/runtime compaction evidence), `T8G4X976ZX5JN30YGNJQA5CMRM` (sandbox admission boundary evidence). |
| **C** | **SHIPPED** | Bun runtime, sandbox declaration enforcement, artifact admission, artifact store, verifier invocation, act tuples, and `action_scored` residual recording are implemented as the first end-to-end runtime lane. The old "Deno permissions" wording is historical; the shipped lane is bun plus substrate sandbox/preflight checks. | `runtime/runtimes/bun.ts`, `runtime/sandbox.ts`, `runtime/artifact_admission.ts`, `runtime/artifact_store.ts`, `runtime/act_tuple.ts`, `runtime/events.ts`; commit `4aa79c1`. | `runtime/runtimes/bun.test.ts`; `runtime/sandbox.test.ts`; `runtime/artifact_admission.test.ts`; `runtime/artifact_store.test.ts`; `runtime/task_dispatcher.test.ts` (`action_predicted -> action -> verifier -> action_scored -> task_committed`). | Events: `T8G4X976ZX5JN30YGNJQA5CMRM` (runtime/sandbox guards), `QFNHFPTX5D5ED6NS34TGRJ3JEM` (bun admission result marker), `AS94GQCP0H3Y17VAKKSJDYKRC8` (dispatcher artifact-seeding test pattern). |
| **D** | **SHIPPED** | Single-cycle opencode bridge, prompt composition, dispatcher, scheduler integration, `fixture_d_count_todos`, cycle-1 enforcement, and silent-exit/stuck-bridge hardening are live. | `runtime/bridge.ts`, `runtime/prompt_composer.ts`, `runtime/task_dispatcher.ts`, `runtime/task_scheduler.ts`, `runtime/fixtures/d_count_todos.test.ts`; commits `4dbbe2c`, `4d08661`. | `runtime/bridge.test.ts` (`fixture_d_count_todos`, real subprocess surfaces, MCP tool surface); `runtime/prompt_composer.test.ts` (`EXIT INVARIANT`, budget sections, retrieval rendering); `runtime/task_dispatcher.test.ts` (cycle-1 enforcement, refinement); `runtime/fixtures/d_count_todos.test.ts`. | Events: `4EAFA894A8194C4CA74F08430C` (exit invariant), `1841BAD728AB439FAE3135A4D3` (hard-task dispatch gate), `AS94GQCP0H3Y17VAKKSJDYKRC8` (dispatcher test lesson). |
| **E** | **SHIPPED** | Parallel scheduling, task DAG topology, refinement/watch edges, directive amendments, and interference deferral are live. | `runtime/task_scheduler.ts`, `runtime/task_topology.ts`, `runtime/watch_edges.ts`, `runtime/amendment_handler.ts`, `runtime/interference.ts`, `substrate/views.ts`; commits `b305719`, `1826363`. | `runtime/task_scheduler.test.ts` (parallel dispatch, caps, fairness, conflict deferral); `runtime/task_topology.test.ts`; `runtime/amendment_handler.test.ts`; `runtime/prompt_composer.test.ts` (`WATCHED OUTPUTS`); `runtime/interference.test.ts`; `tests/contract_amendment_consumer.test.ts`. | Events: `26195910D9BE46D18863ED75EC` (successful apply-gate trajectory compression), `8E3040E6904F426FB1EFCB99A7` (pre-apply adjudication convergence), `T3MZCWYVY10Y571HVJ2YXK17C8` (dispatch status depends on substrate lifecycle rows). |
| **F** | **SHIPPED** | Depth-1 retrieval is live through embedding, retrieval, reranking, prompt sections, and `substrate.search`. Historical provenance is important: HNSW was promised in earlier prose, but the shipped implementation is disk-resident `sqlite-vec` `vec_events` with cosine x posterior reranking and test-dimension fallback. | `runtime/embedder.ts`, `runtime/embedding_index.ts`, `runtime/retrieval.ts`, `substrate/schema.sql`; commits `3aec394`, `9aa9889`, `8014632` (HNSW references aligned to sqlite-vec), `25eb1e9` (32k prompt budget). | `runtime/sqlite_vec.test.ts`; `runtime/embedding_index.test.ts`; `runtime/embedder.test.ts`; `runtime/retrieval.test.ts`; `runtime/prompt_composer.test.ts` (budget and promoted-knowledge fallback). | Events: `WHXHCPVRXS5AV9SWJ21EHMQBZG` (multi-vector retrieval direction), `GG8MEN9MB569H7C55KCT94FJR0` (graph edges augment vector retrieval), `CFY3NB2Q3S5ND5D3C2D22QWQW0` (semantic retrieval replaces hard-coded routing). |
| **G** | **PARTIAL** | `uv` and `camofox-browser` runtime lanes exist and are tested, but live substrate evidence still records preflight/parity gaps around env credentials and sandbox diagnostics, so the phase is not fully closed as a parity claim. | `runtime/runtimes/uv.ts`, `runtime/runtimes/camofox.ts`, `runtime/artifact_admission.ts`; commits `a8fb20a`, `e3cb0e2`. | `runtime/runtimes/uv.test.ts` (inputs, sandbox degradation/enforcement, timeout cleanup); `runtime/runtimes/camofox.test.ts` (wrapper generation, profile mutex, launch gate); `runtime/artifact_admission.test.ts`. | Events: `SDT7PDPYX13J3BMAMANZHG58F4` (bring uv/camofox preflight to bun parity), `VQA4E2HC3H7X16M56JB3PJ2YXW` (`env_requires` parity gap), `W2DSPJWEMH43FBBAWG3N9HS5PC` (resource envelopes versus enforcement-specific sandboxes). |
| **H** | **SHIPPED** | Artifact posterior scoring, promotion, quarantine, retirement, rehabilitation, credit, and registry views are live. `code_artifact_*` event names are retained as historical compatibility names while the registry abstraction is `act_artifact`; the broader recipe/code-artifact terminology sweep is intentionally left to a separate contract. | `runtime/artifact_store.ts`, `runtime/artifact_provenance.ts`, `substrate/views.ts`, `substrate/schema.sql`; commits `f7a8967`, `6998342`, `bfc3ac6`. | `runtime/artifact_store.test.ts` (`maybePromote`, `maybeQuarantine`, `maybeRehabilitate`, full lifecycle); `runtime/artifact_provenance.test.ts`; `tests/act_artifact_rename.test.ts`; `runtime/task_dispatcher.test.ts` (credit pipeline). | Events: `645AWCE0MX5DV6HD96TS46WDKR` (act_artifact registry convergence), `2KC4DZTR6S2KV4T9TWFFH39R08` (remaining physical/API rename boundary), `T8A83QFGHX72996DCVPQV8K93G` (one registry, no parallel definition tables). |
| **I** | **SHIPPED** | Multi-stakeholder state, directive interference, rolling reviews, crisis mode, owner profile/rendering loops, and universal fixture coverage are live. | `runtime/stakeholder_compositor.ts`, `runtime/interference.ts`, `runtime/rolling_reviewer.ts`, `runtime/crisis_mode.ts`, `runtime/fixtures/*`; commits `e1db9a5`, `70627ca`. | `runtime/stakeholder_compositor.test.ts`; `runtime/interference.test.ts`; `runtime/rolling_reviewer.test.ts`; `runtime/crisis_mode.test.ts`; `runtime/fixtures/d_multi_stakeholder.test.ts`; `runtime/fixtures/d_long_horizon_savings.test.ts`; `runtime/fixtures/d_crisis_response.test.ts`; `tests/universal_lessons.test.ts`. | Events: `7D1522710972414B80E9723746` (owner decisions and residual-scored control), `6E5WK4D7W536X7A16480VZ6WSR` (long tasks map to rolling reviews/refinement), `A1326BEB07834D3FB9E23B5E79` (promoted history touching Phase I surfaces). |
| **J** | **SHIPPED** | Tier-0 replay is live through goal/topology matching, confidence thresholds, action/verifier replay, and abort-on-residual semantics. The old `recipes_view` wording is historical; current surfaces use recipe-shaped knowledge/projections and `runtime.replay_recipe`. | `runtime/recipe_replay.ts`, `runtime/task_scheduler.ts`, `runtime/task_dispatcher.ts`, `substrate/views.ts`; commits `c4f2ec8`, `6e5c101`, `2c6ef8f`. | `runtime/recipe_replay.test.ts` (matching, end-to-end replay, multi-step replay, residual abort, confidence updates); `runtime/task_scheduler.test.ts` (`substrate_replay` route); `runtime/task_dispatcher.test.ts` (`Phase J: substrate_replay route commits without calling the bridge`). | Events: `JKE8RES3JX39VBKF2E162HTN9G` (multi-step recipe replay tests), `8JMXED294H03Q81DJR9WPZQMKG` (Tier-0 replay safety boundary), `26195910D9BE46D18863ED75EC` (successful trajectories compress into recipes). |
| **K** | **PARTIAL** | Father is shipped for the original constrained recurring-task contract, but live design evidence says Father v2 is being re-scoped toward event-reactive journaling/pacing and away from planner-era objective selection. The updated Father v2 contract remains open. | `runtime/father.ts`, `runtime/father.test.ts`, `runtime/daemon.ts`; commits `c4f2ec8`, `a439fb2`. | `runtime/father.test.ts` (owner-active yield, no bridge import/opencode call, drift detection/self-suspend, priority/freshness/conflicts, recipe-backed template preference). | Events: `V9AG24HSX53A51BEXM17EQPP84` (Father v2 partially migrated), `MYBBGQ9N0D0DVCYX49FN1HGCEC` (Option gamma redesign), `6XBE0M3NJ91EK1HW7D95272G7M` (drop planner-era responsibilities before closure). |
| **L** | **PARTIAL** | v1 is frozen/archive-only in practice and the migration path was deliberately removed, but the operational owner-freeze/drain/archive state is now governed by owner consent and narrow-safe-scope apply gates rather than a one-time phase switch. No v1 code mutation is bundled here. | Operational contract in `CLAUDE.md`, `docs/operator-install.md`, `docs/ops-guide.md`, and drain/restart code in `runtime/daemon.ts`; commits `722f928`, `4573c59`. | `runtime/daemon.test.ts` (bounded graceful drain, shutdown accounting); `tests/restart_quiescence.test.ts`; `runtime/owner_gate.test.ts`; `cli/apply.test.ts`; `tests/lifecycle_closure_sweep.test.ts`. | Events: `5480522523764A4AB6E0BCCAC7` (v2 does not migrate from v1), `E41351621F3244F58EFE6B19C3` (owner consent autonomous-mode trajectory), `6H3ZQFZMXN7V` (owner-supplied narrow safe scope), `9F9QH3BHX12KQ1YPX0K7WP33NC` (safe additive contracts need current gate evidence). |

Rows marked **PARTIAL** are open contracts, not hidden failures. Phase G needs uv/camofox preflight and sandbox parity closure. Phase K needs the Father v2 re-scope resolved. Phase L remains operationally gated by owner consent and safe-scope evidence. There is still no fine-tuning phase: the substrate's posterior-weighted retrieval over an event log is the compounding mechanism; a small custom model would duplicate it less reliably, and the subscription-CLI-only constraint precludes a custom inference path.
...
23. **Daemon crash with in-memory state.** MCP server state and scheduler queues live in daemon memory. Mitigation: WAL guarantees event durability; the embedding index lives in `sqlite-vec`'s `vec_events` virtual table (disk-resident, no rebuild needed); daemon boot re-arms workers from event log; in-flight brain dispatches use their own SQLite connection and survive daemon restart.
...
2. **Prompt budget value.** Default raised to `DEFAULT_BUDGET_TOKENS = 32000` (2026-05-19) once floor-section enforcement landed and surfaced that load-bearing sections (top_laws + owner_profile + owner_rendering_policy + retrieved_knowledge) legitimately run 10–15k tokens on a warm substrate. 32k matches modern brain context windows (gpt-5.5 128K, opus 4.7 1M) while leaving headroom. Open: at what budget does brain quality plateau, and at what budget does latency become unacceptable on the subscription CLI path?
...
- v2 new (no v1 ancestor): daemon.ts, embedding_index.ts (thin wrapper over `sqlite-vec`'s `vec_events`), external_ingress.ts, dispatch_decider.ts, runtimes/camofox.ts as runtime (not host), refinement-edge depth check inside task_topology.ts.

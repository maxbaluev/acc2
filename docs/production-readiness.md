...
| 6 | major    | OwnerAutonomy drift detection emits `owner_autonomy_drift_detected` but never **self-suspends** (audit-report.md E.5) — the `owner_autonomy_self_suspended` event kind is not even declared in `EventKind`. | M | post-Phase K |
...
| `runtime/owner_autonomy.ts` (~12 mentions) | OwnerAutonomy is Phase K; the Phase K marker is design-doc-correct, but OwnerAutonomy's `owner_autonomy_self_suspended` reservation never materialised (E.5 deferral). | Phase K + post-K | See Top-10 item 6. |
...
`substrate/schema.sql` declares 3 tables: `meta` (k/v), `events` (append-only ledger), `act_artifact` (LATM registry). Design §4.1 names these three. **info — aligned.**
...
- `owner_autonomy_self_suspended` — audit-report.md E.5 reserves this name; never added to `EventKind`. OwnerAutonomy continues to emit `owner_autonomy_drift_detected` instead of suspending. **major.**
...
| `substrate.emit_act_artifact_candidate(...)` | `substrate.emit` → routed to `substrate.admit_artifact` for fixture admission | OK |
| `substrate.emit_directive_amended(...)` | `substrate.amend_directive` | OK (also accepts emit kind=directive_amended) |

Plus 15 tools BEYOND what §13.2 documents: `substrate.embed_text`, `substrate.run_artifact`, `substrate.run_verifier`, `substrate.credit`, `substrate.admit_artifact`, `substrate.open_fixture`, `runtime.dispatch_ready_task`, `runtime.scheduler_tick`, `substrate.record_stakeholder_state`, `substrate.record_interference_edge`, `substrate.open_directive`, `runtime.process_rolling_reviews`, `runtime.owner_autonomy_iterate`, `runtime.detect_owner_autonomy_drift`, `substrate.find_recipe`, `runtime.replay_recipe`, `substrate.register_external_source`, `runtime.recent_events`. All design-justified (added by Phase E-K) but not documented in §13.2 of the design doc. **minor — design doc could be updated to enumerate the full tool surface.**
...
- `runtime/owner_autonomy.ts:122,123` — `OWNER_ACTIVE_WINDOW_MS_DEFAULT = 60_000`, `TEMPLATE_USE_COOLDOWN_MS = 10 * 60 * 1000`. OK.
...
### Top-6: OwnerAutonomy never self-suspends on repeated drift

**Gap.** `audit-report.md` E.5: "OwnerAutonomy currently emits `owner_autonomy_drift_detected` on each offender but does not self-suspend. The reservation for the suspension event kind is `owner_autonomy_self_suspended` — not yet added to `EventKind`."

**Why it blocks production.** OwnerAutonomy runs autonomously every 5 minutes. If it starts emitting forbidden events (anything outside `OWNER_AUTONOMY_ACTION_EVENT_KINDS`), the drift detector logs it but OwnerAutonomy keeps running. An owner who has gone to sleep is not protected from a OwnerAutonomy run amok.

**Fix path.**
- Add `owner_autonomy_self_suspended` to `EventKind` in `substrate/types.ts`.
- In `runtime/owner_autonomy.ts`, after N=3 consecutive `owner_autonomy_drift_detected` events for the same OwnerAutonomy instance, emit `owner_autonomy_self_suspended` and refuse further iteration until an owner-driven `acc owner_autonomy resume`. ~50 LOC.
- `acc owner_autonomy resume` CLI subcommand in `cli/dispatch.ts`. ~25 LOC.

**Test.** Trigger 3 drift events in a row; assert OwnerAutonomy emits `owner_autonomy_self_suspended` and subsequent `owner_autonomyIterate` calls return without action.
...
**Fix path.** Rewrite `acc2/.env.example` to only document acc2-reachable env vars: OPENAI_API_KEY, ACC2_BRIDGE_MODE, ACC2_OPENCODE_MODEL, ACC2_OPENCODE_TIMEOUT_MS, ACC2_OPENCODE_MCP_HANDSHAKE_MS, ACC2_DISABLE_WORKERS (canonical opt-OUT, comma-separated worker names), ACC2_OWNER_AUTONOMY_INTERVAL_MS, V2_DAEMON_PORT, V2_DAEMON_AUX_PORT, V2_MCP_SERVER_URL, ACC2_STATE_DIR, ACC2_DB_PATH, ACC2_EXTERNAL_PUSH_TOKEN, CAMOUFOX_BINARY_PATH, CAMOUFOX_HEADLESS, CAMOUFOX_LOCALE, ACC2_SANDBOX_PROC_ALLOW, SERPER_API_KEY, ACC2_TEST_DB_PATH. ~60 LOC delete + ~30 LOC add.
...
| **runtime — OwnerAutonomy** | post-K maintenance — closes Top-6 | Add `owner_autonomy_self_suspended` event kind; add `acc owner_autonomy resume` command. |

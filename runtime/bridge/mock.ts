...
// ── Claude Code engine mock (symmetric to opencodeQueryMock) ──────────
//
// Hermetic mock for the substrate-spawned Claude Code engine bridge. It emits
// the cc_* lifecycle rows (cc_dispatched → cc_frame_received → cc_dispatch_closed)
// PLUS the same act-artifact admission + action_predicted flow the opencode
// mock produces, WITHOUT launching any real `claude` subprocess — so tests
// exercise the Claude engine's lifecycle + cycle-1/watchdog seams deterministically.
//
// The CC engine attributes its events under the free-string origin
// "claude_code" (SubstrateOrigin is open by design), symmetric to the opencode
// mock's "opencode" origin. The single fixture marker keeps the scaffold small;
// increment 2 (executor selection) can extend the dispatch table as needed.
const CC_ENGINE_ORIGIN = "claude_code" as const;
export const FIXTURE_CC_IMPLEMENT_MARKER = "FIXTURE: fixture_cc_implement_todo_count";

export const claudeCodeQueryMock = async (
  req: BridgeRequest,
  db: Database,
): Promise<BridgeResult> => {
  const emittedEventIds: string[] = [];
  const dispatchId = req.dispatchId ?? `cc_mock_${req.taskId}`;

  emitEvent(db, {
    kind: "bridge_invoked",
    substrate_origin: CC_ENGINE_ORIGIN,
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: { prompt_chars: req.prompt.length, engine: "claude_code" } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });

  // cc_dispatched — spawn-time lifecycle row (subprocess_pid null: no real proc).
  const dispatched = emitEvent(db, {
    kind: "cc_dispatched",
    substrate_origin: CC_ENGINE_ORIGIN,
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: {
      ...
      subprocess_pid: null,
      engine: "claude_code",
      emission_origin: "cc_bridge_mock",
    } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });
  emittedEventIds.push(dispatched.id);

  // Unknown prompts decline with the supported-marker list (symmetric to the
  // opencode mock's mock_bridge_prompt_unrecognized).
  if (!req.prompt.includes(FIXTURE_CC_IMPLEMENT_MARKER)) {
    const supportedMarkers = [FIXTURE_CC_IMPLEMENT_MARKER];
    emitEvent(db, {
      kind: "bridge_failed",
      substrate_origin: CC_ENGINE_ORIGIN,
      directive_id: req.directiveId,
    ...
    emitEvent(db, {
      ...
      directive_id: req.directiveId,
      task_id: req.taskId,
      payload: { dispatch_id: dispatchId, closure_reason: "cc_mock_unrecognized", engine: "claude_code" } as JsonValue,
      invoker: CC_ENGINE_ORIGIN,
    });
    return {
      ok: false,
  ...
  // cc_frame_received — one mock frame proving the lifecycle advanced.
  const frame = emitEvent(db, {
    kind: "cc_frame_received",
    substrate_origin: CC_ENGINE_ORIGIN,
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: { dispatch_id: dispatchId, frame_type: "tool_use", frame_index: 1, engine: "claude_code" } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });
  emittedEventIds.push(frame.id);

  // Admit the canonical action + verifier pair through the SAME admission
  // pipeline the opencode mock uses (reuses FIXTURE_D bodies for parity).
  emitEvent(db, {
    kind: "act_artifact_candidate",
    substrate_origin: CC_ENGINE_ORIGIN,
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: { runtime: "bun", purpose: "fixture_cc_implement_action" } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });
  const actionAdmission = await admitArtifact(
    db,
    {
      ...
      fixtureInput: { target_path: "/nonexistent-admission-probe" } as JsonValue,
      fixtureExpectedResidualBelow: 0.2,
      name: "fixture_cc_implement_action",
    },
    (ev) => {
      const out = emitEvent(db, {
        ...ev,
        directive_id: ev.directive_id ?? req.directiveId,
        task_id: ev.task_id ?? req.taskId,
        invoker: ev.invoker ?? CC_ENGINE_ORIGIN,
      });
      emittedEventIds.push(out.id);
    },
  );
  if (!actionAdmission.ok) {
    emitEvent(db, {
      ...
      directive_id: req.directiveId,
      task_id: req.taskId,
      payload: { reason: `action_admission_failed:${actionAdmission.reason}`, engine: "claude_code" } as JsonValue,
      invoker: CC_ENGINE_ORIGIN,
    });
    emitEvent(db, {
      kind: "cc_dispatch_closed",
      ...
      directive_id: req.directiveId,
      task_id: req.taskId,
      payload: { dispatch_id: dispatchId, closure_reason: "cc_mock_action_admission_failed", engine: "claude_code" } as JsonValue,
      invoker: CC_ENGINE_ORIGIN,
    });
    return { ok: false, reason: { kind: "subprocess_crash", stderr_tail: actionAdmission.reason } };
  }
  ...
  emitEvent(db, {
    ...
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: { runtime: "bun", purpose: "fixture_cc_implement_verifier" } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });
  const verifierAdmission = await admitArtifact(
    db,
    {
      ...
      fixtureInput: { result: { count: 0 } } as JsonValue,
      fixtureExpectedResidualBelow: 0.2,
      name: "fixture_cc_implement_verifier",
    },
    (ev) => {
      const out = emitEvent(db, {
        ...ev,
  ...
  if (!verifierAdmission.ok) {
    emitEvent(db, {
      ...
      directive_id: req.directiveId,
      task_id: req.taskId,
      payload: { reason: `verifier_admission_failed:${verifierAdmission.reason}`, engine: "claude_code" } as JsonValue,
      invoker: CC_ENGINE_ORIGIN,
    });
    emitEvent(db, {
      kind: "cc_dispatch_closed",
      ...
      directive_id: req.directiveId,
      task_id: req.taskId,
      payload: { dispatch_id: dispatchId, closure_reason: "cc_mock_verifier_admission_failed", engine: "claude_code" } as JsonValue,
      invoker: CC_ENGINE_ORIGIN,
    });
    return { ok: false, reason: { kind: "subprocess_crash", stderr_tail: verifierAdmission.reason } };
  }
  ...
  const predicted = emitEvent(db, {
    ...
    predicted_residual: 0.05,
    payload: {
      intent: "count files containing TODO in target directory (cc engine mock)",
      target_path: req.fixtureTargetPath ?? ".",
      engine: "claude_code",
    } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  ...
  // cc_dispatch_closed — deterministic close, symmetric to brain_dispatch_closed.
  const closed = emitEvent(db, {
    kind: "cc_dispatch_closed",
    substrate_origin: CC_ENGINE_ORIGIN,
    directive_id: req.directiveId,
    task_id: req.taskId,
    payload: { dispatch_id: dispatchId, closure_reason: "cc_mock_completed", engine: "claude_code" } as JsonValue,
    invoker: CC_ENGINE_ORIGIN,
  });
  emittedEventIds.push(closed.id);

  return {
    ok: true,
    final_response: "fixture_cc_implement action_predicted emitted (claude_code mock)",
    usage: { tokens: 0 },
    emitted_event_ids: emittedEventIds,
  };
};

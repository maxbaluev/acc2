...
describe("renderingAuditWorkerTick", () => {
  test("no rendered rows → zero work", () => {
    const db = openDb(":memory:");
    runViews(db);
    const r = renderingAuditWorkerTick(db);
    expect(r).toEqual({ scanned: 0, audited: 0, flagged: 0, cleaned: 0, skipped_existing_feedback: 0 });
  });

  test("clean primary render → auto_verifier_clean feedback emitted with residual 0", () => {
    const db = openDb(":memory:");
    runViews(db);
  ...
  test("primary render with an ID and substrate jargon → auto_verifier feedback with residual > threshold", () => {
    const db = openDb(":memory:");
    runViews(db);
  ...
  test("idempotency: re-running the tick on the same render does NOT double-emit feedback", () => {
    const db = openDb(":memory:");
    runViews(db);
  ...
  test("rendered_text missing or non-string → auto_verifier_skipped feedback, not a verifier score", () => {
    const db = openDb(":memory:");
    runViews(db);
    const ev = emitEvent(db, {
      kind: "rendered_owner_message_recorded",
  ...
  test("detail_drawer audience relaxes ID leak — clean band even when text contains ULIDs", () => {
    const db = openDb(":memory:");
    runViews(db);

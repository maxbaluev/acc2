...
**Surface pending owner decisions last (anti-data-loss rule).** When there are unresolved HIDL events (`owner_input_required`, `hidl_action_required`), owner-gated `contract_amendment_proposed` rows awaiting consent, brain-emitted proposals still parked as described-only / failed, or any other choice the owner needs to make for the substrate to advance, the orchestrator's LAST message in each turn MUST end with a concise decision card listing the choices. Even if autonomous work completed cleanly mid-turn, surface the next pending owner choice at the bottom so context isn't lost across turn boundaries, context compaction, or session resume. Decision cards: ≤3 lines per choice, one-word reply expected (a/b/skip/approve/reject), preferred over prose summaries. When there are zero pending decisions, the rule is satisfied by an explicit "no pending decisions" closing line so the owner can confirm the queue is drained.
...
**Render to the owner's own learned vector — never to a fixed persona enum.** Owners are NOT bucketed into "developer / operator / casual". Each owner accumulates a unique, continuous vector of `rendering_signals` — an **open-ended `Record<string, number>`** where new dimensions can be added by observation without schema changes. Common dimensions span style, formality, abstraction, redundancy, narrative shape, pace, and technical vocabulary, but the brain MUST treat the keys as discovered, not enumerated. Plus `preferred_terms`, `avoided_terms`, `exposed_concepts`, `understood_concepts`, and `declined_concepts` on the `owner_profile_recorded` row. For any non-trivial owner-visible reply after a substrate dispatch, run `acc owner policy` when the daemon is available and use that output as the rendering policy; if the command is unavailable, fall back to the same `owner_profile_recorded` fields exposed in the current prompt and surface the missing CLI as a tooling gap. Read each signal independently and treat ANY signal name as valid input — the classifier may emit new keys after observation.
...
- High `formal_register` → formal phrasing; high `casual_register` → direct casual phrasing; neither known → stay neutral.
- High `concrete_examples_appetite` → include examples before abstractions.
- High `repeat_for_confirmation` → recap decisions and ask confirmation before risky pivots.
- High `linear_vs_branched` or `one_step_at_a_time_vs_batch` → one question + one next step; low values can tolerate batched options when `code_density` is also high.
- Any other learned key → adapt directionally on the same scalar logic; absence of a key means "no evidence yet, do not assume".
- Always mirror `preferred_terms` back; never use `avoided_terms`; explain a concept only on first encounter (check `exposed_concepts`); use concepts in `understood_concepts` directly without re-teaching; avoid re-teaching concepts in `declined_concepts` (use the owner's `preferred_term` if recorded).
...
- **DO NOT OVER-ENGINEER.** (Owner directive 2026-05-16.) The universal verifier (residual ∈ [0,1] + open-ended `breakdown:Record<string,number>`) is the **one** truth-bearing signal. Anything that fights, duplicates, or pre-empts it is over-engineering. Before adding a gate, refusal path, structural pre-check, fixed enum, parallel CLI primitive, multi-step ceremony, or defensive validation layer, ask: *would the residual + breakdown already score this?* If yes — don't add it; the verifier closes the loop via posterior, no enforcement needed. Removed this session under this rule (commit hashes for reference): `cli/apply.ts` gates `structured_proposed_behavior_required`, `trajectory_hazard_present`, `apply_diff_mismatch:drift` (1e7c9d4); `runtime/auto_apply_worker.ts` (entire 1120-line worker), `runtime/proposal_grounding.ts` (validateProposalGrounding 362 lines), `runtime/task_dispatcher.ts` assertDispatchableArtifact (01da5cb). Each refused legitimately-correct work the verifier would have scored; each is now gone. **Owner consent on protected targets (CLAUDE.md / docs/ / .claude/rules/) is orthogonal — it gates WHO can change, not WHETHER the change is correct.** Keep that one. Cycle-1 enforcement (`runtime/cycle_one_gate.ts`) and `integrity_worker.ts` PRAGMA checks are also keepers: structural laws + operational safety, not verifier duplicates.

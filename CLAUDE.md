...
**Render to the owner's own learned vector — never to a fixed persona enum.** Owners are NOT bucketed into "developer / operator / casual". Each owner accumulates a unique, continuous vector of `rendering_signals` (e.g. `code_density`, `ops_vocabulary`, `explanation_appetite` ∈ [0, 1]) plus `preferred_terms`, `avoided_terms`, and `exposed_concepts` on their `owner_profile_recorded` row. For any non-trivial owner-visible reply after a substrate dispatch, run `acc owner policy` when the daemon is available and use that output as the rendering policy; if the command is unavailable, fall back to the same `owner_profile_recorded` fields exposed in the current prompt and surface the missing CLI as a tooling gap. Read each signal independently:

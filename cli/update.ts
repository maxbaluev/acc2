...
export type UpdateEnv = { out:(line:string)=>void; err:(line:string)=>void; spawnSync:typeof spawnSync; startDaemon:()=>Promise<void>; stopDaemon:()=>Promise<boolean>; healthOk:()=>Promise<boolean>; dbPath:string; };
...
export const defaultUpdateEnv = (): UpdateEnv => ({ out:(line)=>console.log(line), err:(line)=>console.error(line), spawnSync, startDaemon: realStartDaemon, stopDaemon: realStopDaemon, healthOk: realHealthOk, dbPath: resolveDbPath() });
...
export const runUpdate = async (argv: string[], env: UpdateEnv = defaultUpdateEnv()): Promise<number> => {
  ...
  env.out("pre-update health snapshot:"); const pre = await collectChecks(defaultDoctorEnv()); for (const line of renderReport(pre, computeReadiness(pre), false)) env.out("  " + line);
